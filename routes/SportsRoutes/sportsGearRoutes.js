...
const sportsGear = require("../../models/Sports/sportsGearModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        sportsGear.find((err,data)=>{
            if(err){
...
router.get("/searchSportsGear/:key", async(req,res)=>{
    let result = await sportsGear.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        sportsGear.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new sportsGear(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        sportsGear.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        sportsGear.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
