...
const sportShoe = require("../../models/Sports/sportShoesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        sportShoe.find((err,data)=>{
            if(err){
...
router.get("/searchSportShoes/:key", async(req,res)=>{
    let result = await sportShoe.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        sportShoe.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new sportShoe(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        sportShoe.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        sportShoe.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
