...
const fitnessEquipments = require("../../models/Sports/fitnessEquipmentsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        fitnessEquipments.find((err,data)=>{
            if(err){
...
router.get("/searchFitnessEquipments/:key", async(req,res)=>{
    let result = await fitnessEquipments.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        fitnessEquipments.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new fitnessEquipments(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        fitnessEquipments.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        fitnessEquipments.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
