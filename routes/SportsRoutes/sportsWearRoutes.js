...
const sportsWear = require("../../models/Sports/sportsWearModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        sportsWear.find((err,data)=>{
            if(err){
...
router.get("/searchSportsWear/:key", async(req,res)=>{
    let result = await sportsWear.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        sportsWear.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new sportsWear(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        sportsWear.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        sportsWear.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
