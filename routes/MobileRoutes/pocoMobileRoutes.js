...
const pocoMobile = require("../../models/Mobiles/pocoMobileModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        pocoMobile.find((err,data)=>{
            if(err){
...
router.get("/searchPoco/:key", async(req,res)=>{
    let result = await pocoMobile.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        pocoMobile.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new pocoMobile(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        pocoMobile.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        pocoMobile.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
