...
const oppoMobile = require("../../models/Mobiles/oppoMobileModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        oppoMobile.find((err,data)=>{
            if(err){
...
router.get("/searchOppo/:key", async(req,res)=>{
    let result = await oppoMobile.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        oppoMobile.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new oppoMobile(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        oppoMobile.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        oppoMobile.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
