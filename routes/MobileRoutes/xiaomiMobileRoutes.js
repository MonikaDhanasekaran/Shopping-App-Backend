...
const xiaomiMobile = require("../../models/Mobiles/xiaomiMobileModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        xiaomiMobile.find((err,data)=>{
            if(err){
...
router.get("/searchXiaomi/:key", async(req,res)=>{
    let result = await xiaomiMobile.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        xiaomiMobile.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new xiaomiMobile(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        xiaomiMobile.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        xiaomiMobile.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
