...
const vivoMobile = require("../../models/Mobiles/vivoMobileModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        vivoMobile.find((err,data)=>{
            if(err){
...
router.get("/searchVivo/:key", async(req,res)=>{
    let result = await vivoMobile.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        vivoMobile.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new vivoMobile(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        vivoMobile.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        vivoMobile.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
