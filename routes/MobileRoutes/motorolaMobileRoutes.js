...
const motorolaMobile = require("../../models/Mobiles/motorolaMobileModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        motorolaMobile.find((err,data)=>{
            if(err){
...
router.get("/searchMotorola/:key", async(req,res)=>{
    let result = await motorolaMobile.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        motorolaMobile.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new motorolaMobile(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        motorolaMobile.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        motorolaMobile.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
