...
const appleMobile = require("../../models/Mobiles/appleMobileModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        appleMobile.find((err,data)=>{
            if(err){
...
router.get("/searchApple/:key", async(req,res)=>{
    let result = await appleMobile.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        appleMobile.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new appleMobile(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        appleMobile.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        appleMobile.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
