...
const samsungMobile = require("../../models/Mobiles/samsungMobileModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        samsungMobile.find((err,data)=>{
            if(err){
...
router.get("/searchSamsung/:key", async(req,res)=>{
    let result = await samsungMobile.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        samsungMobile.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new samsungMobile(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        samsungMobile.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        samsungMobile.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
