...
const realmeMobile = require("../../models/Mobiles/realmeMobileModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        realmeMobile.find((err,data)=>{
            if(err){
...
router.get("/searchRealme/:key", async(req,res)=>{
    let result = await realmeMobile.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        realmeMobile.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new realmeMobile(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        realmeMobile.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        realmeMobile.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
