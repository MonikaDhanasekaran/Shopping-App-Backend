...
const recentSale = require("../models/recentSaleModel");
const router = express.Router();

router.get("/get", (req, res) => {
    try {
        recentSale.find((err, data) => {
            if (err) {
...
router.get("/searchRecentSale/:key", async (req, res) => {
    let result = await recentSale.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try {
        recentSale.findOne({ _id: req.params.dataID }, (err, data) => {
            if (err) {
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new recentSale(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req, res) => {
    try {
        recentSale.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err, data) => {
            if (err) {
...
router.delete("/delete/:dataID", auth.authenticateUser, (req, res) => {
    try {
        recentSale.deleteOne({ _id: req.params.dataID }, (err, data) => {
            if (err) {
