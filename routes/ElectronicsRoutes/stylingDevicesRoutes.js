...
const stylingDevices = require("../../models/Electronics/stylingDevicesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        stylingDevices.find((err,data)=>{
            if(err){
...
router.get("/searchStylingDevices/:key", async(req,res)=>{
    let result = await stylingDevices.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        stylingDevices.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new stylingDevices(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        stylingDevices.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        stylingDevices.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
