...
const smartWearable = require("../../models/Electronics/smartWearablesModels");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        smartWearable.find((err,data)=>{
            if(err){
...
router.get("/searchSmartWearables/:key", async(req,res)=>{
    let result = await smartWearable.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        smartWearable.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new smartWearable(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        smartWearable.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        smartWearable.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
