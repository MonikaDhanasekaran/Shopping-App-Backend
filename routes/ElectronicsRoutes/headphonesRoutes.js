...
const headphones = require("../../models/Electronics/headphonesModels");
...
router.get("/searchHeadphones/:key", async(req,res)=>{
    let result = await headphones.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        headphones.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new headphones(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        headphones.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        headphones.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
