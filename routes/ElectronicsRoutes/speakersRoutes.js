...
const speaker = require("../../models/Electronics/speakersModel");
...
router.get("/searchSpeakers/:key", async(req,res)=>{
    let result = await speaker.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        speaker.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new speaker(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        speaker.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        speaker.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
