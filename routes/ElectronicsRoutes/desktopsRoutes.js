...
const desktop = require("../../models/Electronics/desktopsModels");
...
router.get("/searchDesktops/:key", async(req,res)=>{
    let result = await desktop.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        desktop.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new desktop(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        desktop.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        desktop.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
