...
const laptop = require("../../models/Electronics/laptopsModel");
...
router.get("/searchLaptops/:key", async(req,res)=>{
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        laptop.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new laptop(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        laptop.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        laptop.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
