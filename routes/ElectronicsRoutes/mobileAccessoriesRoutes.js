...
const mobileAccessory = require("../../models/Electronics/mobileAccessoriesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        mobileAccessory.find((err,data)=>{
            if(err){
...
router.get("/searchMobileAccessories/:key", async(req,res)=>{
    let result = await mobileAccessory.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        mobileAccessory.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new mobileAccessory(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        mobileAccessory.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        mobileAccessory.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
