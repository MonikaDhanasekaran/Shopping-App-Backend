...
const computer = require("../../models/Electronics/computerAccessoriesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        computer.find((err,data)=>{
            if(err){
...
router.get("/searchComputerAccessories/:key", async(req,res)=>{
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        computer.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new computer(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        computer.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        computer.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
