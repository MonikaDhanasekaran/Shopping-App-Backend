...
const powerbank = require("../../models/Electronics/powerbankModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        powerbank.find((err,data)=>{
            if(err){
...
router.get("/searchPowerbanks/:key", async(req,res)=>{
    let result = await powerbank.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        powerbank.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new powerbank(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        powerbank.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        powerbank.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
