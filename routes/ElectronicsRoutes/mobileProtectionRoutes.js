...
const mobileProtection = require("../../models/Electronics/mobileProtectionModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        mobileProtection.find((err,data)=>{
            if(err){
...
router.get("/searchMobileProtection/:key", async(req,res)=>{
    let result = await mobileProtection.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        mobileProtection.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new mobileProtection(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        mobileProtection.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        mobileProtection.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
