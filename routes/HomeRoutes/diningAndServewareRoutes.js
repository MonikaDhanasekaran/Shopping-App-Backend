...
const diningAndServeware = require("../../models/Home/diningAndServewareModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        diningAndServeware.find((err,data)=>{
            if(err){
...
router.get("/searchDiningAndServeware/:key", async(req,res)=>{
    let result = await diningAndServeware.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        diningAndServeware.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new diningAndServeware(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        diningAndServeware.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        diningAndServeware.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
