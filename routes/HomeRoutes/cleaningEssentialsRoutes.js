...
const cleaningEssentials = require("../../models/Home/cleaningEssentialsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        cleaningEssentials.find((err,data)=>{
            if(err){
...
router.get("/searchCleaningEssentials/:key", async(req,res)=>{
    let result = await cleaningEssentials.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        cleaningEssentials.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new cleaningEssentials(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        cleaningEssentials.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        cleaningEssentials.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
