...
const gardeningEssentials = require("../../models/Home/gardeningEssentialsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        gardeningEssentials.find((err,data)=>{
            if(err){
...
router.get("/searchGardeningEssentials/:key", async(req,res)=>{
    let result = await gardeningEssentials.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        gardeningEssentials.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new gardeningEssentials(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        gardeningEssentials.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        gardeningEssentials.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
