...
const homeFurnishing = require("../../models/Home/homeFurnishingModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        homeFurnishing.find((err,data)=>{
            if(err){
...
router.get("/searchHomeFurnishing/:key", async(req,res)=>{
    let result = await homeFurnishing.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        homeFurnishing.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new homeFurnishing(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        homeFurnishing.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        homeFurnishing.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
