...
const lightning = require("../../models/Home/lightningModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        lightning.find((err,data)=>{
            if(err){
...
router.get("/searchLightning/:key", async(req,res)=>{
    let result = await lightning.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        lightning.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new lightning(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        lightning.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        lightning.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
