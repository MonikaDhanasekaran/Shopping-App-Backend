...
const homeDecor = require("../../models/Home/homeDecorModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        homeDecor.find((err,data)=>{
            if(err){
...
router.get("/searchHomeDecor/:key", async(req,res)=>{
    let result = await homeDecor.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        homeDecor.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new homeDecor(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        homeDecor.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        homeDecor.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
