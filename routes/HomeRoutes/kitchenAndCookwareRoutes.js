...
const kitchenAndCookware = require("../../models/Home/kitchenAndCookwareModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        kitchenAndCookware.find((err,data)=>{
            if(err){
...
router.get("/searchKitchenAndCookware/:key", async(req,res)=>{
    let result = await kitchenAndCookware.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        kitchenAndCookware.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new kitchenAndCookware(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        kitchenAndCookware.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        kitchenAndCookware.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
