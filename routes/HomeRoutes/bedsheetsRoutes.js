...
const bedsheet = require("../../models/Home/bedsheetsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        bedsheet.find((err,data)=>{
            if(err){
...
router.get("/searchBedsheets/:key", async(req,res)=>{
    let result = await bedsheet.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        bedsheet.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new bedsheet(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        bedsheet.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        bedsheet.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
