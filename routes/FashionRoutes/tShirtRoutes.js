...
const tShirt = require("../../models/Fashion/tShirtsModel");
...
router.get("/searchTShirts/:key", async(req,res)=>{
    let result = await tShirt.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        tShirt.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new tShirt(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        tShirt.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        tShirt.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
