...
const shoes = require("../../models/Fashion/sportShoesModel");
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        shoes.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new shoes(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        shoes.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        shoes.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
