...
const jean = require("../../models/Fashion/jeansModel");
...
router.get("/searchJeans/:key", async(req,res)=>{
    let result = await jean.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        jean.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new jean(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        jean.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        jean.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
