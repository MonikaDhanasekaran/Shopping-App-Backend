...
const trackPants = require("../../models/Fashion/trackPantsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        trackPants.find((err,data)=>{
            if(err){
...
router.get("/searchTrackpants/:key", async(req,res)=>{
    let result = await trackPants.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        trackPants.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new trackPants(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        trackPants.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        trackPants.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
