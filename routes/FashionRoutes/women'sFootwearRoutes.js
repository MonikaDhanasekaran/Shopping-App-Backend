...
const footwear = require("../../models/Fashion/women'sFootwearModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        footwear.find((err,data)=>{
            if(err){
...
router.get("/searchWomenFootwear/:key", async(req,res)=>{
    let result = await footwear.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        footwear.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new footwear(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        footwear.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        footwear.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
