...
const bags = require("../../models/Fashion/luggageAndBagsModel");
...
router.get("/searchLuggagesAndBags/:key", async(req,res)=>{
    let result = await bags.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        bags.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new bags(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        bags.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        bags.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
