...
const sareesAndKurtas = require("../../models/Fashion/sareesAndKurtasModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        sareesAndKurtas.find((err,data)=>{
            if(err){
...
router.get("/searchSareesAndKurtas/:key", async(req,res)=>{
    let result = await sareesAndKurtas.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        sareesAndKurtas.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new sareesAndKurtas(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        sareesAndKurtas.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        sareesAndKurtas.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
