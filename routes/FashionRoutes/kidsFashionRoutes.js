...
const kidsFashion = require("../../models/Fashion/kidsFashionModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        kidsFashion.find((err,data)=>{
            if(err){
...
router.get("/searchKidsFashion/:key", async(req,res)=>{
    let result = await kidsFashion.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        kidsFashion.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new kidsFashion(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        kidsFashion.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        kidsFashion.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
