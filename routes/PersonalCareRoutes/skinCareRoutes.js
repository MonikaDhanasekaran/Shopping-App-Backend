...
const skinCare = require("../../models/PersonalCare/skinCareModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        skinCare.find((err,data)=>{
            if(err){
...
router.get("/searchSkinCare/:key", async(req,res)=>{
    let result = await skinCare.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        skinCare.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new skinCare(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        skinCare.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        skinCare.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
