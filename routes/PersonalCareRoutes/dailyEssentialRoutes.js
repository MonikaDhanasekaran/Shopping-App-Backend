...
const dailyEssential = require("../../models/PersonalCare/dailyEssentialsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        dailyEssential.find((err,data)=>{
            if(err){
...
router.get("/searchDailyEssentials/:key", async(req,res)=>{
    let result = await dailyEssential.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        dailyEssential.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new dailyEssential(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        dailyEssential.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        dailyEssential.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
