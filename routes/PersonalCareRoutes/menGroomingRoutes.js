...
const menGrooming = require("../../models/PersonalCare/menGroomingModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        menGrooming.find((err,data)=>{
            if(err){
...
router.get("/searchMenGrooming/:key", async(req,res)=>{
    let result = await menGrooming.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        menGrooming.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new menGrooming(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        menGrooming.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        menGrooming.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
