...
const fragrance = require("../../models/PersonalCare/fragrancesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        fragrance.find((err,data)=>{
            if(err){
...
router.get("/searchFragrance/:key", async(req,res)=>{
    let result = await fragrance.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        fragrance.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new fragrance(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        fragrance.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        fragrance.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
