...
const oralCare = require("../../models/PersonalCare/oralCareModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        oralCare.find((err,data)=>{
            if(err){
...
router.get("/searchOralCare/:key", async(req,res)=>{
    let result = await oralCare.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        oralCare.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new oralCare(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        oralCare.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        oralCare.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
