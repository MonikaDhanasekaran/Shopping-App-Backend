...
const makeup = require("../../models/PersonalCare/makeupModel");
...
router.get("/searchMakeup/:key", async(req,res)=>{
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        makeup.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new makeup(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        makeup.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        makeup.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
