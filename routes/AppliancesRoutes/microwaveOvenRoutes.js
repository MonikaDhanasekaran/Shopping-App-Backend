...
const oven = require("../../models/Appliances/microwaveOvensModel");
...
router.get("/searchMicrowaveOven/:key", async(req,res)=>{
    let result = await oven.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        oven.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new oven(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        oven.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        oven.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
