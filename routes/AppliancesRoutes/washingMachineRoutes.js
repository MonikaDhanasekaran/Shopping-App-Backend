...
const washingMachine = require("../../models/Appliances/washingMachineModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        washingMachine.find((err,data)=>{
            if(err){
...
router.get("/searchWashingMachine/:key", async(req,res)=>{
    let result = await washingMachine.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        washingMachine.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new washingMachine(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        washingMachine.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        washingMachine.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
