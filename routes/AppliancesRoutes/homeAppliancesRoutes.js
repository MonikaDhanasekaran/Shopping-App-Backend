...
const home = require("../../models/Appliances/homeAppliancesModel");
...
router.get("/searchHomeAppliances/:key", async(req,res)=>{
    let result = await home.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        home.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new home(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        home.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        home.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
