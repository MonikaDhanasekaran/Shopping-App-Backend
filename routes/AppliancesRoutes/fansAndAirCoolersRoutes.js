...
const fansAndAirCoolers = require("../../models/Appliances/fansAndAirCoolersModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        fansAndAirCoolers.find((err,data)=>{
            if(err){
...
router.get("/searchFansAndAirCoolers/:key", async(req,res)=>{
    let result = await fansAndAirCoolers.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        fansAndAirCoolers.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new fansAndAirCoolers(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        fansAndAirCoolers.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        fansAndAirCoolers.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
