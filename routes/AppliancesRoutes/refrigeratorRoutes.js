...
const refrigerator = require("../../models/Appliances/refrigeratorsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        refrigerator.find((err,data)=>{
            if(err){
...
router.get("/searchRefrigerator/:key", async(req,res)=>{
    let result = await refrigerator.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        refrigerator.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new refrigerator(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        refrigerator.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        refrigerator.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
