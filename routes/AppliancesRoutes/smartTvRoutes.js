...
const smartTv = require("../../models/Appliances/smartTvModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        smartTv.find((err,data)=>{
            if(err){
...
router.get("/searchSmartTV/:key", async(req,res)=>{
    let result = await smartTv.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        smartTv.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new smartTv(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        smartTv.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        smartTv.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
