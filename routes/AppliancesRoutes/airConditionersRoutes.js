...
const ac = require("../../models/Appliances/airConditionersModel");
...
router.get("/searchAirConditioners/:key", async(req,res)=>{
    let result = await ac.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        ac.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        ac.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        ac.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
