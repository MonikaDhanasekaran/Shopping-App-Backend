...
const kitchen = require("../../models/Appliances/kitchenAppliancesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        kitchen.find((err,data)=>{
            if(err){
...
router.get("/searchKitchenAppliances/:key", async(req,res)=>{
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        kitchen.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new kitchen(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        kitchen.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        kitchen.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
