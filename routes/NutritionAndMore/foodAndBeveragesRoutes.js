...
const foodAndBeverages = require("../../models/NutritionAndMore/foodAndBeveragesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        foodAndBeverages.find((err,data)=>{
            if(err){
...
router.get("/searchFoodAndBeverages/:key", async(req,res)=>{
    let result = await foodAndBeverages.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        foodAndBeverages.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new foodAndBeverages(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        foodAndBeverages.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        foodAndBeverages.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
