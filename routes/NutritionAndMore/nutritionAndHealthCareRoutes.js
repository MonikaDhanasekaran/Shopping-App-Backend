...
const nutritionAndHealthCare = require("../../models/NutritionAndMore/nutritionAndHealthCareModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        nutritionAndHealthCare.find((err,data)=>{
            if(err){
...
router.get("/searchNutritionAndHealthCare/:key", async(req,res)=>{
    let result = await nutritionAndHealthCare.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        nutritionAndHealthCare.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new nutritionAndHealthCare(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        nutritionAndHealthCare.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        nutritionAndHealthCare.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
