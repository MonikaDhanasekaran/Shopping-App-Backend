...
const sportsAndFitness = require("../../models/NutritionAndMore/sportsAndFitnessModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        sportsAndFitness.find((err,data)=>{
            if(err){
...
router.get("/searchSportsAndFitness/:key", async(req,res)=>{
    let result = await sportsAndFitness.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        sportsAndFitness.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new sportsAndFitness(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        sportsAndFitness.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        sportsAndFitness.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
