...
const petSupplies = require("../../models/NutritionAndMore/petSuppliesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        petSupplies.find((err,data)=>{
            if(err){
...
router.get("/searchPetSupplies/:key", async(req,res)=>{
    let result = await petSupplies.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        petSupplies.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new petSupplies(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        petSupplies.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        petSupplies.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
