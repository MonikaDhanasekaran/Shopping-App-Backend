...
const autoAccessories = require("../../models/NutritionAndMore/autoAccessoriesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        autoAccessories.find((err,data)=>{
            if(err){
...
router.get("/searchAutoAccessories/:key", async(req,res)=>{
    let result = await autoAccessories.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        autoAccessories.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new autoAccessories(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        autoAccessories.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        autoAccessories.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
