...
const guidanceForParents = require("../../models/ToysAndBabyCare/guidanceForParentsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        guidanceForParents.find((err,data)=>{
            if(err){
...
router.get("/searchGuidanceForParents/:key", async(req,res)=>{
    let result = await guidanceForParents.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        guidanceForParents.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new guidanceForParents(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        guidanceForParents.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        guidanceForParents.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
