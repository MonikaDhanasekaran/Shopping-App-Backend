...
const babyCareEssentials = require("../../models/ToysAndBabyCare/babyCareEssentialsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        babyCareEssentials.find((err,data)=>{
            if(err){
...
router.get("/searchBabyCareEssentials/:key", async(req,res)=>{
    let result = await babyCareEssentials.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        babyCareEssentials.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new babyCareEssentials(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        babyCareEssentials.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        babyCareEssentials.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
