...
const toysAndGames = require("../../models/ToysAndBabyCare/toysAndGamesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        toysAndGames.find((err,data)=>{
            if(err){
...
router.get("/searchToysAndGames/:key", async(req,res)=>{
    let result = await toysAndGames.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        toysAndGames.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new toysAndGames(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        toysAndGames.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        toysAndGames.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
