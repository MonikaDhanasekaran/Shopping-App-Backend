...
const stationeryAndSchoolSupplies = require("../../models/ToysAndBabyCare/stationeryAndSchoolSuppliesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        stationeryAndSchoolSupplies.find((err,data)=>{
            if(err){
...
router.get("/searchStationeryAndSchoolSupplies/:key", async(req,res)=>{
    let result = await stationeryAndSchoolSupplies.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        stationeryAndSchoolSupplies.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new stationeryAndSchoolSupplies(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        stationeryAndSchoolSupplies.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        stationeryAndSchoolSupplies.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
