...
const beds = require("../../models/Furnitures/bedsModel");
...
router.get("/searchBeds/:key", async(req,res)=>{
    let result = await beds.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        beds.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new beds(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        beds.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        beds.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
