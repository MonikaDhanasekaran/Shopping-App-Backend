...
const diningTables = require("../../models/Furnitures/diningTablesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        diningTables.find((err,data)=>{
            if(err){
...
router.get("/searchDiningTables/:key", async(req,res)=>{
    let result = await diningTables.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        diningTables.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new diningTables(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        diningTables.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        diningTables.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
