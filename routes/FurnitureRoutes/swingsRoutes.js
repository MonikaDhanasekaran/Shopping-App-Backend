...
const swings = require("../../models/Furnitures/swingsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        swings.find((err,data)=>{
            if(err){
...
router.get("/searchSwings/:key", async(req,res)=>{
    let result = await swings.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        swings.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new swings(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        swings.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        swings.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
