...
const tvUnits = require("../../models/Furnitures/tvUnitsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        tvUnits.find((err,data)=>{
            if(err){
...
router.get("/searchTvUnits/:key", async(req,res)=>{
    let result = await tvUnits.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        tvUnits.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new tvUnits(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        tvUnits.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        tvUnits.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
