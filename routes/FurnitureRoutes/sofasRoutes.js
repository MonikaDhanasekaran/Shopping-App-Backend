...
const sofas = require("../../models/Furnitures/sofasModel");
...
router.get("/searchSofas/:key", async(req,res)=>{
    let result = await sofas.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        sofas.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new sofas(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        sofas.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        sofas.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
