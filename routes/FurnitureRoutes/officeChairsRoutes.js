...
const officeChairs = require("../../models/Furnitures/officechairsModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        officeChairs.find((err,data)=>{
            if(err){
...
router.get("/searchOfficeChairs/:key", async(req,res)=>{
    let result = await officeChairs.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        officeChairs.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new officeChairs(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        officeChairs.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        officeChairs.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
