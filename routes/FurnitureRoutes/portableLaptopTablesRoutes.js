...
const portableLaptopTable = require("../../models/Furnitures/portableLaptopTablesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        portableLaptopTable.find((err,data)=>{
            if(err){
...
router.get("/searchPortableLaptopTables/:key", async(req,res)=>{
    let result = await portableLaptopTable.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        portableLaptopTable.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new portableLaptopTable(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        portableLaptopTable.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        portableLaptopTable.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
