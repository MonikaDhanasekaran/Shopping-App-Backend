...
const recliners = require("../../models/Furnitures/reclinersModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        recliners.find((err,data)=>{
            if(err){
...
router.get("/searchRecliners/:key", async(req,res)=>{
    let result = await recliners.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        recliners.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new recliners(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        recliners.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        recliners.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
