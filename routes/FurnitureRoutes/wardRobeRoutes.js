...
const wardRobes = require("../../models/Furnitures/wardRobesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        wardRobes.find((err,data)=>{
            if(err){
...
router.get("/searchWardrobes/:key", async(req,res)=>{
    let result = await wardRobes.find({
        "$or": [
            {
...
router.get('/getOne/:dataID', (req, res) => {
    try{
        wardRobes.findOne({_id: req.params.dataID}, (err, data) => {
            if(err){
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new wardRobes(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        wardRobes.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        wardRobes.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
